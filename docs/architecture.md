...
## 컴포넌트 개요
- **Fetcher**: NEWSDATA.io API 호출, 뉴스 데이터 수집, 중복/변경 감지, 큐에 적재.
...
## 데이터 모델
- `sources`: `id PK`, `name`, `api_type (newsdata)`, `active`, `last_fetched_at`.
...
## 설정/환경 변수(예시)
- `GEMINI_API_KEY`, `NEWSDATA_API_KEY`, `DATABASE_URL`, `RATE_LIMIT_PER_MIN`, `ALLOWED_ORIGINS`.
